...
export default function ForgotPassword() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true);
      toast.success("Nếu email tồn tại, BKQuiz đã gửi liên kết đặt lại mật khẩu.");
    } catch (cause) {
      const message = cause instanceof ApiRequestError ? cause.message : "Không thể gửi yêu cầu. Vui lòng thử lại.";
      setError(message);
      toast.error(message);
    } finally {
      setLoading(false);
            ...
            {!submitted ? (
                ...
                <form className="mt-7 space-y-5" onSubmit={submit}>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <Input id="forgot-email" className="pl-9" type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" required placeholder="mssv@hust.edu.vn" />
                    </div>
                  </div>
              ...
              <div className="py-4 text-center">
                ...
                <p className="mt-3 text-sm leading-6 text-[#6B7280]">
                  Vui lòng kiểm tra hộp thư đến và thư mục spam. Thời hạn của liên kết được ghi trong email.
                </p>
                <Link to="/login">

...
export default function Admin() {
  ...
  return <div className="min-h-screen bg-[#F7F7F8]">
    <header className="flex items-center justify-between border-b bg-white px-6 py-4"><span className="text-sm font-black uppercase tracking-wide text-[#6B7280]">Quản trị hệ thống</span><div className="flex items-center gap-3 text-sm"><b>{user?.username}</b><Button variant="outline" onClick={() => void logout()}>Đăng xuất</Button></div></header>
    <div className="mx-auto grid max-w-[1500px] gap-6 p-6 md:grid-cols-[220px_1fr]">
      ...
      <main className="min-w-0">
        ...
        {tab === "content" && <div className="space-y-3"><div className="flex gap-2">{(["topics","quizzes","classrooms"] as const).map(type=><Button key={type} variant={contentType===type?"primary":"outline"} onClick={()=>setContentType(type)}>{type}</Button>)}</div>{content.error?<Card className="p-6 text-red-600">{message(content.error)}</Card>:content.data?.map(item=><Card key={item.id} className="flex items-center justify-between gap-3 p-4"><div><b>{item.title}</b><p className="text-sm text-gray-500">{item.moderation_status}{item.moderation_reason?` · ${item.moderation_reason}`:""}</p>{item.ai_validation_status === "WARNING" && <p className="mt-1 text-sm font-medium text-amber-700">AI: cần kiểm tra · {item.ai_validation_warning_count ?? 0} cảnh báo</p>}</div><Button size="sm" variant={item.moderation_status==="HIDDEN"?"outline":"danger"} onClick={()=>mutate.mutate(()=>adminApi.moderate(contentType,item.id,item.moderation_status!=="HIDDEN",item.moderation_status!=="HIDDEN"?"Vi phạm chính sách nội dung":""))}>{item.moderation_status==="HIDDEN"?"Khôi phục":"Ẩn"}</Button></Card>)}</div>}

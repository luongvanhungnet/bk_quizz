      ...
      <main className="mx-auto max-w-6xl px-5 py-14 lg:px-8">
          ...
          {plans.map((plan) => (
            <Card key={plan.name} className={`relative p-6 ${plan.highlighted ? "border-[#C8102E] shadow-xl" : ""}`}>
              ...
              <Button
                ...
                onClick={() => toast.success(`Đã chọn gói ${plan.name}`)}
              >
                Chọn gói
              </Button>
            </Card>
        ...
        <Card className="mt-10 grid gap-6 bg-[#FFF4D9] p-6 md:grid-cols-[1fr_auto] md:items-center">
          <div>
            ...
            <p className="mt-2 text-sm text-[#6B7280]">Bản demo này chưa nối thanh toán, nhưng UI đã sẵn sàng cho flow nâng cấp.</p>
          </div>
          <Link to="/dashboard">
            <Button variant="secondary">Vào demo workspace</Button>
          </Link>
        </Card>
      </main>
    </div>
